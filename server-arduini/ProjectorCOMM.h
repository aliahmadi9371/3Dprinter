...
#ifndef PROJECTORCOMM_H
#define PROJECTORCOMM_H
...
typedef struct projectorStruct00 {
	...
	int  Model;
	int  I2CAddr; // For I2C COMM (NQM)
    int  VID;     // For HID COMM (FIREBIRD)
    int  PID;     // For HID COMM (FIREBIRD)
...
public:
	ProjectorCOMM();
    void SetIntensityMinMax(int MinIntensity,int MaxIntensity,int MinIntensitySensor,int MaxIntensitySensor);
	int  ConnectToProjector(int Model);
	int  DisConncetFromProjector(void);
	void ProjectorOnOff(bool Enable);
	void EnableIntensity(bool Enable);
    int SetIntensity(int Percent);
    void SetIntensityVal(int value);
    void SetIntensitySensor(int Percent);
    ...
    int HIDReadData (unsigned char *rbuffer,int len);
    int HIDWriteData (unsigned char *wbuffer, int len);
