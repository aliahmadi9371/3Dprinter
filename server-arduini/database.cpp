...
{
    qDebug() << "DataBase: callback: database callback argc: " << argc;
//    cout<< " database callback argc:"<< argc <<endl;
    int i;
    for(i = 0; i<argc; i++) {
        QString tmp = argv[i] ? argv[i] : "NULL";
        qDebug() << "DataBase: callback: " << azColName[i] << ", " << tmp ;
...
bool DataBase::openConnection()
{
  ...
  if (exit != SQLITE_OK) {
        qDebug() << "DataBase: openConnection: Error Open connection";
          cerr << "Error Open connection" << endl;
  } else {
      isOpen = true;
      qDebug() << "DataBase: openConnection: connection opened";
...
bool DataBase::createTable(Schema tableSchema)
{
    ...
    qDebug() << "DataBase: createTable: createTable: " << QString::fromStdString(sql);
//    cout<<endl<<"createTable : " << sql << endl;
    exit = sqlite3_exec(DB, sql.c_str(), callback, 0, &zErrMsg);

      if( exit != SQLITE_OK ){
         qDebug() << "DataBase: createTable: createTable SQL error: " << zErrMsg << ", EXIT CODE :"<< exit ;
         cerr << "createTable SQL error: " << zErrMsg << ", EXIT CODE :"<< exit<< endl;
         sqlite3_free(zErrMsg);
         return false;
      } else {
         qDebug() << "DataBase: createTable: " << QString::fromStdString(tableSchema.tableName) << " table created successfully";
...
{
    qDebug() << "DataBase: insert: ********************insert";
//     cout<<"********************insert : " << endl;
    int exit = 0;
    if(DB == NULL) {
        qDebug() << "DataBase: insert: insert DB is nul";
//        cout<< "insert DB is null"<<endl;
    }
    if(!isOpen) {return false;}

    char *zErrMsg = 0;
    qDebug() << "DataBase: insert: DataBase  insert: " << QString::fromStdString(sql_command);
//    cout<<"DataBase  insert : " << sql_command << endl;
    exit = sqlite3_exec(DB, sql_command.c_str(), callback, 0, &zErrMsg);

      if( exit != SQLITE_OK ){
          qDebug() << "DataBase: insert: insert SQL error , EXIT CODE : " << exit;
         cerr << "insert SQL error: " <<  ", EXIT CODE :"<< exit << endl;
         sqlite3_free(zErrMsg);
         return false;
      } else {
         qDebug() << "DataBase: insert: table inserted successfully";
...
bool DataBase::update(string sql_command)
{
    ...
    char *zErrMsg = 0;
    qDebug() << "DataBase: insert: update: " << QString::fromStdString(sql_command);
//    cout<<"update : " << sql_command << endl;
    exit = sqlite3_exec(DB, sql_command.c_str(), callback, 0, &zErrMsg);

      if( exit != SQLITE_OK ){
          qDebug() << "DataBase: insert: update SQL error: " << zErrMsg << ", EXIT CODE :"<< exit;
         cerr << "update SQL error: " << zErrMsg << ", EXIT CODE :"<< exit<< endl;
         sqlite3_free(zErrMsg);
         return false;
      } else {
          qDebug() << "DataBase: insert: table update successfully: " << QString::fromStdString(sql_command);
          qDebug() << "DataBase: insert: " << zErrMsg;
...
bool DataBase::remove(string sql_command)
{
    ...
    char *zErrMsg = 0;
    qDebug() << "DataBase: insert: remove: " << QString::fromStdString(sql_command);
//    cout<<"remove : " << sql_command << endl;
    exit = sqlite3_exec(DB, sql_command.c_str(), callback, 0, &zErrMsg);

      if( exit != SQLITE_OK ){
          qDebug() << "DataBase: remove: remove SQL error: " << zErrMsg << ", EXIT CODE :"<< exit;
         cerr << "remove SQL error: " << zErrMsg<< ", EXIT CODE :"<< exit << endl;
         sqlite3_free(zErrMsg);
         return false;
      } else {
          qDebug() << "DataBase: remove: row removed successfully";
          qDebug() << "DataBase: remove: " << zErrMsg;
...
bool DataBase::findMotionById(string sql_command, MotionConfig *temp)
{
    ...
    if(DB == NULL) {
        qDebug() << "DataBase: findMotionById: findMotionById DB is null";
//        cout<< "findMotionById DB is null"<<endl;
    }
    if(!isOpen) {
        qDebug() << "DataBase: findMotionById: findById connection is not open";
//        cout<<"findById connection is not open";
        return false;
    }
    ...
    bool successfullyFinded = false;
        while (!done) {
            qDebug() << "DataBase: findMotionById: findMotionById";
//            cout<<"findMotionById";
            rc = sqlite3_step (stmt);
            switch (rc) {
              case SQLITE_ROW:
                qDebug() << "DataBase: findMotionById: SQLITE_ROW";
//                cout<<" SQLITE_ROW"<<endl;
              ...
              case SQLITE_DONE:
                qDebug() << "DataBase: findMotionById: SQLITE_DONE";
//                cout<<" SQLITE_DONE"<<endl;
              ...
              default:
                ...
                qDebug() << "DataBase: findMotionById: Failed";
//                cout<<" Failed"<<endl;
...
bool DataBase::findAllMotion(string sql_command, vector<MotionConfig> *mConfV)
{
    ...
    MotionConfig mConf;
    if(DB == NULL) {
        qDebug() << "DataBase: findAllMotion: findAllMotion DB is null";
//        cout<< "findAllMotion DB is null"<<endl;
    }
    if(!isOpen) {
        qDebug() << "DataBase: findAllMotion: findById connection is not open";
//        cout<<"findById connection is not open";
        return false;
    }
    sqlite3_mutex_enter(sqlite3_db_mutex(DB));
    ...
    bool successfullyFinded = false;
        while (!done) {
            ...
            qDebug() << "DataBase: findAllMotion: In select while";
//            printf("In select while\n");
            ...
            switch (rc) {
              ...
              default:
                sqlite3_finalize(stmt);
                qDebug() << "DataBase: findAllMotion: Failed";
                fprintf(stderr, "Failed.\n");
...
bool DataBase::findJobById(string sql_command, __Job3D *temp)
{
    ...
    const unsigned char* text;
    if(DB == NULL) {
        qDebug() << "DataBase: findJobById: findJobById DB is null";
//        cout<< "findJobById DB is null"<<endl;
    }
    if(!isOpen) {
        qDebug() << "DataBase: findJobById: findById connection is not open";
//        cout<<"findById connection is not open";
        return false;
    }
    sqlite3_mutex_enter(sqlite3_db_mutex(DB));
    ...
    bool successfullyFinded = false;
        while (!done) {
            qDebug() << "DataBase: findJobById: findJobById";
//            cout<<"findJobById ";
            rc = sqlite3_step (stmt);
            switch (rc) {
              case SQLITE_ROW:
                qDebug() << "DataBase: findJobById: SQLITE_ROW";
//                cout<<"SQLITE_ROW"<<endl;
                temp->id = static_cast<int>(sqlite3_column_int(stmt, 0)) ;
                temp->Name = string( reinterpret_cast<const char*>( sqlite3_column_text(stmt, 1) ) );
              ...
              case SQLITE_DONE:
                 ...
                 qDebug() << "DataBase: findJobById: SQLITE_DONE";
//                 cout<<"SQLITE_DONE"<<endl;
                 break;

              default:
                sqlite3_finalize(stmt);
                qDebug() << "DataBase: findJobById: Failed";
//                cout<<"Failed"<<endl;
                fprintf(stderr, "Failed.\n");
                sqlite3_mutex_leave(sqlite3_db_mutex(DB));
                return false;
...
bool DataBase::findAllJob(string sql_command, vector<__Job3D> *j3DV)
{
    ...
    const unsigned char* text;
    if(DB == NULL) {
        qDebug() << "DataBase: findAllJob: findAllJob DB is null";
//        cout<< "findAllJob DB is null"<<endl;
    }
    if(!isOpen) {
        qDebug() << "DataBase: findAllJob: findAllJob connection is not open";
//        cout<<"findAllJob connection is not open";
        return false;
    }
    sqlite3_mutex_enter(sqlite3_db_mutex(DB));
    sqlite3_prepare_v2(DB, sql_command.c_str(), -1, &stmt, 0);
    ...
    bool successfullyFinded = false;
        while (!done) {
            ...
            qDebug() << "DataBase: findAllJob: findAllJob step one";
//            printf("findAllJob step one\n");
            ...
            switch (rc) {
              ...
              default:
                qDebug() << "DataBase: findAllJob: Failed";
                fprintf(stderr, "Failed.\n");
                sqlite3_finalize(stmt);
                sqlite3_mutex_leave(sqlite3_db_mutex(DB));
                return false;
...
bool DataBase::findMachineConfigById(string sql_command, MachineConfig *temp)
{
    ...
    const unsigned char* text;
    if(DB == NULL) {
        qDebug() << "DataBase: findMachineConfigById: findMachineConfigById DB is null";
//        cout<< "findMachineConfigById DB is null"<<endl;
    }
    if(!isOpen) {
        qDebug() << "DataBase: findMachineConfigById: findMachineConfigById connection is not open";
//        cout<<"findMachineConfigById connection is not open";
        return false;
    }
    sqlite3_mutex_enter(sqlite3_db_mutex(DB));
    sqlite3_prepare_v2(DB, sql_command.c_str(), -1, &stmt, 0);
    ...
    bool successfullyFinded = false;
        while (!done) {
            qDebug() << "DataBase: findMachineConfigById: In select while";
//            printf("In select while\n");
            ...
            switch (rc) {
              ...
              default:
                qDebug() << "DataBase: findMachineConfigById: Failed";
                fprintf(stderr, "Failed.\n");
                sqlite3_finalize(stmt);
                return false;
            }
...
bool DataBase::findAllMachineConfig(string sql_command, vector<MachineConfig> *mConfV)
{
    ...
    MachineConfig mConf;
    if(!isOpen) {
        qDebug() << "DataBase: findAllMachineConfig: findAllMachineConfig connection is not open";
//        cout<<"findAllMachineConfig connection is not open";
        return false;
    }
    if(DB == NULL) {
        qDebug() << "DataBase: findAllMachineConfig: findAllMachineConfig DB is null";
//        cout<< "findAllMachineConfig DB is null"<<endl;
    }
    sqlite3_mutex_enter(sqlite3_db_mutex(DB));
    sqlite3_prepare_v2(DB, sql_command.c_str(), -1, &stmt, 0);
    ...
    bool successfullyFinded = false;
        while (!done) {
            ...
            qDebug() << "DataBase: findAllMachineConfig: In select while";
//            printf("In select while\n");
            ...
            switch (rc) {
              ...
              default:
                qDebug() << "DataBase: findAllMachineConfig: Failed";
                fprintf(stderr, "Failed.\n");
                sqlite3_mutex_leave(sqlite3_db_mutex(DB));
                return false;
            }
...
bool DataBase::findMaterialById(string sql_command, MaterialConfig *temp)
{
    ...
    const unsigned char* text;
    if(DB == NULL) {
        qDebug() << "DataBase: findMaterialById: findMaterialById DB is null";
//        cout<< "findMaterialById DB is null"<<endl;
    }
    if(!isOpen) {
        qDebug() << "DataBase: findMaterialById: findMaterialById connection is not open";
//        cout<<"findById connection is not open";
        return false;
    }
    sqlite3_mutex_enter(sqlite3_db_mutex(DB));
    ...
    bool successfullyFinded = false;
        while (!done) {
            qDebug() << "DataBase: findMaterialById: findMaterialById";
//            cout<< "findMaterialById";
            ...
            switch (rc) {
              case SQLITE_ROW:
                ...
                qDebug() << "DataBase: findMaterialById: SQLITE_ROW";
//                cout<< "SQLITE_ROW"<<endl;
                successfullyFinded = true;
              ...
              case SQLITE_DONE:
                qDebug() << "DataBase: findMaterialById: SQLITE_DONE";
//                 cout<< "SQLITE_DONE"<<endl;
                 done = true;
                 break;
              ...
              default:
//                cout<< "Failed"<<endl;
                qDebug() << "DataBase: findMaterialById: Failed";
                sqlite3_finalize(stmt);
                fprintf(stderr, "Failed.\n");
                sqlite3_mutex_leave(sqlite3_db_mutex(DB));
                return false;
...
bool DataBase::findAllMaterial(string sql_command, vector<MaterialConfig> *mConfV)
{
    ...
    const unsigned char* text;
    if(DB == NULL) {
        qDebug() << "DataBase: findAllMaterial: findAllMaterial DB is null";
//        cout<< "findAllMaterial DB is null"<<endl;
    }
    if(!isOpen) {
        qDebug() << "DataBase: findAllMaterial: findAllMaterial connection is not open";
//        cout<<"findAllMaterial connection is not open";
        return false;
    }
    ...
    bool successfullyFinded = false;
        while (!done) {
            ...
            switch (rc) {
              ...
              default:
                qDebug() << "DataBase: findAllMaterial: Failed: " << rc;
                sqlite3_finalize(stmt);
